...
def user_registration():
    ...
    response = user_collection.find_one(
        {"email_id": email_id, "contact_number": contact_number}
    )
    if not response:
...
def user_login():
    ...
    if response:
        access_token = create_access_token(
            identity=user_name, expires_delta=datetime.timedelta(minutes=10)
        )
        return jsonify(
            {
                "status": "success",
...
def region_options():
    ...
    region_values = [
        col.replace("region_", "") for col in model.feature_names_in_ if "region_" in col
    ]
